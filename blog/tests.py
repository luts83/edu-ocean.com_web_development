...
# ----------------------------------------------------------------------------
def create_category(name='영어', description=''):
...
class TestModel(TestCase):
    ...
    # ----------------------------------------------------------------------------
    def test_tag(self):
        tag_000 = create_tag(name='영어')
        tag_001 = create_tag(name='중국어')
...
class TestView(TestCase):
    ...
    # ----------------------------------------------------------------------------
    def check_nav(self, soup):
        nav_bar = soup.find('div', id='nav_bar')
        self.assertIn('선원 모집 중', nav_bar.text)
        self.assertIn('내 배 들어가기', nav_bar.text)

    # ----------------------------------------------------------------------------
    def check_right_side(self, soup):
        category_card = soup.find('div', id='category-card')
        self.assertIn('영어 (1)', category_card.text)
    ...
    def test_post_list_no_post(self):
        ...
        self.assertEqual(title.text, 'EduOcean')
        self.check_nav(soup)
        self.assertEqual(Post.objects.count(), 0)
        self.assertIn('아직 게시물이 없습니다.', body.text)
    # ----------------------------------------------------------------------------
    def test_post_list_with_post(self):
        tag_000 = create_tag('영어')
        ...
        post_001 = create_post(
            ...
            author=self.author_000,
            category=create_category(name='영어'),
        )

        post_001.tags.add(tag_000)
        post_001.save()
        ...
        self.assertIn(post_000.title, soup.text)

        post_000_read_more_btn = soup.find('a', id='read-more-post-{}'.format(post_000.pk))
        ...
        self.assertIn('영어', main_div.text)

        post_card_000 = soup.find('a', id='tag_name01')
        self.assertIn('#영어', post_card_000.text)  # Tag가 해당 post의 card마다 있다.
    # ----------------------------------------------------------------------------
    ...
    def test_post_detail(self):
        category_eng = create_category(name='영어')
        post_000 = create_post(
            title='Hi there',
            content='Hello World. We are the world.',
            author=self.author_000,
        ...
        self.assertEqual(title.text, '{} - EduOcean'.format(post_000.title))
        self.check_nav(soup)
        ...
        # Tag
        self.assertIn('#영어', main_div.text)  # Tag가 해당 post의 card마다 있다.

        self.assertIn(category_eng.name, main_div.text)  # category가 main_div에 있다.
    ...
    def test_post_list_by_category(self):
        ...
        response = self.client.get(category_eng.get_absolute_url())
        self.assertEqual(response.status_code, 200)
        ...
        self.assertIn(category_eng.name, main_div.text)
    ...
    def test_post_list_no_category(self):
        ...
        self.assertNotIn(category_eng.name, main_div.text)
    ...
    def test_tag_page(self):
        ...
        main_div = soup.find('div', id='main-div')
        blog_h1 = soup.find('a', id='tag_name01')
